...
This folder contains ready-to-use telemetry assets for `deviser`:
...
1. `deviser` running with OpenTelemetry enabled:
...
2. OpenTelemetry Collector reachable from `deviser`
...
3. Upload `dashboards/deviser-overview.dashboard.json`
4. Select your Prometheus data source
5. Save

## Example Panels Included

- Startup success and failure counters (last hour)
- Loop iteration success/failure rate
- Partition ensure latency (p50/p95)
- Partition maintain latency (p50/p95)
- Purge latency (p50/p95)
- Purge throughput (rows/s)
- Dropped partitions (expired vs quota)
- Partition storage (total bytes + freed bytes)
- Discovered partition count
...
4. Point `deviser` to the Collector service:
